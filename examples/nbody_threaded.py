#!/usr/bin/env python3
"""
N-Body Threaded Demo

Physics concepts demonstrated:
- Parallel computation for physics
- Multi-threaded simulation
- Performance optimization techniques
- Same physics as nbody_benchmark.py

This simulation is identical to nbody_benchmark but uses threading
to update objects in parallel. Compare performance with the
...
- Discuss Amdahl's law and speedup limits
- Compare with single-threaded performance
- Explore threading overhead
"""
...
def create_simulation():
    ...
    # Split objects for parallel processing
    system.split_for_threading(num_threads=2)
...
def main():
    ...
    pygame.display.set_caption(f"N-Body Threaded - {NUM_BODIES} Bodies (Parallel)")
    ...
    while loop:
        ...
        # Draw bodies from split lists
        for chunk in system._split_objects:
            for obj in chunk:
        ...
        # Update physics using threads
        if running:
            start = time.time()
            system.update_threaded()
            elapsed = time.time() - start
            print(f"Frame time: {elapsed:.4f}s (threaded)")
